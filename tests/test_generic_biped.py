...
def validate_json_structure(json_data: Dict[str, Any]) -> None:
    ...
    def check_str_value(value: str, expected_value: str) -> None:
        ...
        ), f"Wrong value: '{value}' (expected '{expected_value}')"
    ...
    if "fluff" in json_data:
        ...
        check_keys(
            json_data["fluff"]["systemmanufacturer"],
            ["chassis", "engine", "armor", "communications", "targeting"],
        )
...
def validate_mtf_conversion(mtf_file: Path):
    ...
    assert expected_keys.issubset(
        json_data.keys()
    ), f"Missing keys in {mtf_file.name}: {expected_keys - json_data.keys()}"
