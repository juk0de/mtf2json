...
def validate_data_vs_reference(
    json_reference: Dict[str, Any], json_data: Dict[str, Any]
) -> None:
    """
        ...
        for key in d1:
            ...
            # lists
                ...
                if not isinstance(d2[key], list):
                    raise AssertionError(
                        f"Type mismatch at '{path + key}': expected list, found {type(d2[key])}."
                    )
                if len(d1[key]) != len(d2[key]):
                    raise AssertionError(
                        f"Length mismatch at '{path + key}': expected {len(d1[key])}, found {len(d2[key])}."
                    )
